...
# --- Confusion Matrix 시각화 수정 버전 ---
        cm = confusion_matrix(all_y_true, all_y_pred, labels=target_classes)
        plt.figure(figsize=(12, 10)) # 도화지 크기를 약간 키워 여유 공간 확보

        # 1. annot_kws로 히트맵 내부 숫자 크기 조절
        ax = sns.heatmap(cm, 
                         annot=True, 
                         fmt='d', 
                         cmap='Blues', 
                         ...
                         annot_kws={"size": 15, "weight": "bold"}) # 숫자 크기 및 굵기

        # 2. 제목(Title) 폰트 크기 확대
        plt.title(f'Confusion Matrix - {model_name}', fontsize=20, pad=25, fontweight='bold')

        # 3. 축 이름(Label) 폰트 크기 확대
        plt.xlabel('Predicted Label', fontsize=16, labelpad=15)
        plt.ylabel('True Label', fontsize=16, labelpad=15)

        # 4. 축 눈금(Ticks - 클래스명) 폰트 크기 및 각도 조절
