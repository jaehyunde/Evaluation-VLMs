...
# Sweep Parameters (참고용 – 실제 실행은 wandb.config에서 받음)
ALPHA_VALUES = [i / 10.0 for i in range(1, 10)]  # 0.1 ~ 0.9
N_RUNS = 10
BASE_SEED = 42

print(f"🚀 Device: {DEVICE}")
print(f"🔁 Sweep plan (reference): Alpha = {ALPHA_VALUES}, Runs per alpha = {N_RUNS}")
...
# =====================
def main():
    # 1) W&B run 시작 (wandb agent 가 config.alpha, config.run_id 를 채워줌)
    wandb.init(project="RatioTrain")

    alpha = getattr(wandb.config, "alpha", None)
    if alpha is None:
        # sweep이 아닌 단독 실행 시 기본값
    ...
    run_id = getattr(wandb.config, "run_id", None)
    if run_id is None:
        run_id = 0
    run_id = int(run_id)
    ...
    seed = BASE_SEED + alpha_idx * 1000 + run_id
    set_seed(seed)

    # config 안에 seed/하이퍼파라미터 기록
    wandb.config.update(
        {
            "alpha": alpha,
            "run_id": run_id,
    ...
    # 결과 누적 & 요약/그래프 갱신
    # =====================
    df_path = os.path.join(OUT_DIR, "reports", "wandb_sweep_all_results.csv")
    summary_path = os.path.join(OUT_DIR, "reports", "wandb_sweep_summary.csv")
    plot_path = os.path.join(OUT_DIR, "reports", "wandb_sweep_f1_vs_alpha.png")
    ...
    print(summary)
    print(f"📊 Saved all results to {df_path}")
    print(f"📊 Saved summary to {summary_path}")

    # F1 vs alpha 그래프 저장
    plt.figure()
    plt.errorbar(summary["alpha"], summary["f1_mean"], yerr=summary["f1_std"], fmt="-o")
    plt.xlabel("alpha")
    ...
    plt.close()
    print(f"🖼 Saved F1 vs alpha plot to {plot_path}")
